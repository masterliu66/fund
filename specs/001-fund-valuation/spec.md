...
### User Story 4 - 管理基金配置列表 (Priority: P1)

一名系统管理员需要维护系统中展示的基金配置，包括：
- 查看所有基金配置的列表
- 添加新的基金配置（基金代码、名称、类型）
- 编辑现有基金配置的信息
- 删除不需要的基金配置
...
**Why this priority**: 这是系统管理的基础功能，决定了哪些基金会在前端列表中展示，是整个系统的数据来源管理。

**Independent Test**:  
管理员可以独立访问基金配置管理页面，完成增删改查操作，不依赖基金估值分析功能。

**Acceptance Scenarios**:

1. **Given** 管理员访问基金配置管理页面，  
   **When** 系统加载完成，  
   **Then** 页面应展示所有基金配置的列表，包括基金代码、名称、类型、创建时间等信息。

2. **Given** 管理员点击"添加基金"按钮，  
   **When** 填写完整的基金信息并提交，  
   **Then** 系统应验证数据格式，成功添加新配置并刷新列表。

3. **Given** 管理员选择某只基金进行编辑，  
   **When** 修改基金信息并保存，  
   **Then** 系统应更新配置信息，并在列表中反映最新状态。

4. **Given** 管理员删除某只基金配置，  
   **When** 确认删除操作，  
   **Then** 系统应软删除该配置（标记为已删除），该基金不再在前端列表中显示。
...
- 当管理员尝试添加重复的基金代码时，系统如何处理？
- 当删除的基金配置有关联的历史数据时，如何处理数据完整性？
...
- **FR-008**: 系统 MUST 提供基金配置管理界面，允许管理员查看、添加、编辑、删除基金配置，并支持按基金类型筛选。
...
- **FundConfig**: 表示系统中的基金配置信息，包括基金代码、名称、类型等基础配置，用于管理哪些基金在系统中展示。
...
- **SC-005**: 管理员可以在 2 分钟内完成基金配置的增删改查操作，包括查找、编辑和删除操作。
